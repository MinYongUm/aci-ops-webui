...
설치 완료 후 브라우저에서 접속하면 로그인 페이지(`/login`)로 이동합니다.
초기 계정으로 로그인 후 초기 설정 페이지(`/setup`)에서 APIC 접속 정보를 입력하면 대시보드를 사용할 수 있습니다.
...
```bash
# 1. docker-compose.release.yml 다운로드
...
# 2. 볼륨 마운트 대상 파일 사전 생성 (Linux/Mac)
touch config.yaml users.yaml .secret_key

# 2. 볼륨 마운트 대상 파일 사전 생성 (Windows PowerShell)
# New-Item config.yaml, users.yaml, .secret_key -ItemType File

# 3. 컨테이너 실행
docker compose -f docker-compose.release.yml up -d
```

실행 후 브라우저에서 `http://localhost:8001` 접속 → 로그인 → APIC 설정(`/setup`) → 대시보드 사용.
...
APIC 계정(APIC에 직접 접속하는 계정)과 앱 로그인 계정은 별도입니다.
`/login`에서는 앱 계정(위 기본 계정)을 사용하고, `/setup`에서 APIC 계정을 입력합니다.
...
| GET /setup | 초기 설정 페이지 (인증 후 접근, config.yaml 미존재 시 자동 리다이렉트) |
...
├── .github/
│   └── workflows/
│       └── ci.yml               # GitHub Actions CI + Docker Hub push (v* 태그 시)
├── .flake8                      # flake8 설정
├── .env.example                 # 환경변수 템플릿
├── Dockerfile                   # 컨테이너 이미지
├── docker-compose.yml           # 로컬 개발 환경 (로컬 빌드)
├── docker-compose.release.yml   # 사용자 배포용 (Docker Hub 이미지 pull)
...
└── tests/
    ...
    └── test_api.py              # API 단위 테스트 (104 passed, 1 skipped)
```
...
v1.9.5 기준: 104 passed, 1 skipped
