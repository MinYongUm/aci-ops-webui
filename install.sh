...
clear
divider
echo -e "${BOLD}  ACI Ops WebUI — Installer v1.9.4${RESET}"
...
if [[ "${CONFIG_SIZE}" -gt 0 ]]; then
    ...
    echo -e "    password: aci-ops-admin"
    echo -e "    (최초 로그인 후 반드시 비밀번호를 변경하세요)"
fi
...
echo ""
echo -e "  컨테이너 상태 확인:"
echo -e "    ${COMPOSE_CMD} -f ${INSTALL_DIR}/${COMPOSE_FILE} ps"
echo ""
divider
echo ""